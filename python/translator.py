...
# Dictionnaries to map braille to english
...
# Dictionnaries to map english to braille
...
for i in range(1, len(sys.argv)):
    difference_set = set(sys.argv[i]).difference(set(braille_characters))
    if len(difference_set) == 0:
        # It is Braille
        translator_braille(sys.argv[i])
        if i + 1 != len(sys.argv):
            print(" ", end='')
    else:
        # It is English
        translator_english(sys.argv[i])
        if i + 1 != len(sys.argv):
            print("......", end='')
